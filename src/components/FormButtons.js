...
export const SubmitButton = ({ formikProps, label = 'Save', ...buttonProps }) => {
  ...
  return (
    <Button color="primary" size="sm" {...buttonProps} type="submit" disabled={!dirty || isSubmitting} onClick={handleSubmit}>
      <span>{label}</span>
    </Button>
  );
};
...
export const ResetButton = ({ formikProps, label = 'Reset', ...buttonProps }) => {
  ...
  return (
    <Button color="danger" size="sm" {...buttonProps} type="reset" disabled={!dirty || isSubmitting} onClick={handleReset}>
      <span>{label}</span>
    </Button>
  );
};
